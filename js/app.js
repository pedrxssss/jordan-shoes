/* IMPORT */
import { price, cleanRealFormat } from "./util.js";
...
/* CART DETAILS */
const freightSpan = document.querySelector(".freight-value");
const descountSpan = document.querySelector(".descount-value");
const subTotalSpan = document.querySelector(".subtotal");
const purchaseSpan = document.querySelector(".purchase-value");
...
const updateCartAndItems = () => {
  ...
  subTotalSpan.innerHTML = price.format(total);
  purchaseSpan.innerHTML = price.format(
    total + freightValue - total * (descountValue / 100)
  );
...
const isValidProduct = (product) => {
  return (
    product.id && product.product_name && product.product_model && product.price
  );
};
...
freightSpan.innerHTML = price.format(freightValue);
descountSpan.innerHTML = `${descountValue}%`;
